...
tree = Tree(
    tree={
        ...
        "leaf": [],
        "stem": [],
        dataset_name: ["flower_head", "leaf", "stem"],
    },
    start_dir=data_dir,
...
class TreeCNN(pl.LightningModule):
    ...
    def __init__(
        ...
        def get_constants(is_leaf: bool, is_root: bool) -> "TreeCNN.Constants":
            return TreeCNN.Constants(
                ...
                crop_size=128 if is_leaf and not is_root else 512,
            )
        ...
        self.dataset_transform = torchvision.transforms.Compose(
            [
                torchvision.transforms.RandomAffine(
                    0, translate=(0.25, 0.25) if is_leaf and not is_root else (0.1, 0.1)
                ),
                ...
                OrderedDict(
                    {
                        "cnn": nn.Sequential(
                            OrderedDict(
                                {
                                    "conv2d_1": factory_conv2d(3, 8, 5, pool_size=4),
                                    "conv2d_2": factory_conv2d(8, 4, 3, pool_size=4),
                                    "conv2d_3": factory_conv2d(4, 1, 3, pool_size=2),
                                }
                            )
                            if is_leaf
                            else OrderedDict(
                                {
                                    ...
                                    "conv2d_2": factory_conv2d(1, 1, 3, pool_size=2),
                                }
                            )
                        ),
    ...
    def setup(self, stage=None):
        ...
        def balance_dataset(dataset, normal=True):
            if normal:
                ...
                groups = {
                    ...
                    for i in groupby(
                        enumerate(sorted(dataset.samples)), lambda i: i[1][1]
                    )
                }
        ...
        if stage == "test":  # or stage is None:
            ...
            test = balance_dataset(all_test, normal=not self.is_root)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.recall(preds, y.unsqueeze(1))
        self.precision(preds, y.unsqueeze(1))
...
gen = natsorted(
    pathlib.Path(
        f"/mnt/milchior/serdaylv/data/imagenet/ILSVRC/Data/CLS-LOC/val/n02102040"
    ).iterdir()
)

im = Image.open(gen[49])
display(im)
hm = model(im, transform=True, cnn=True).squeeze(dim=(0, 1)).cpu().detach().numpy()
sns.heatmap(hm, vmax=10, vmin=0, cmap="viridis", square=True)
...
np.argmax(
    [
        model(im, transform=True, cnn=False).cpu().detach().numpy()
        for i in range(1, 50)
        if (
            im := torchvision.transforms.functional.center_crop(
                Image.open(gen[i]), (512, 512)
            )
        )
    ]
)
