    ...
    def __init__(
        ...
        def get_constants(is_leaf: bool, is_root: bool) -> "TreeCNN.Constants":
            return TreeCNN.Constants(
                learning_rate=1e-2 if is_leaf else 1e-3,
                batch_size=256,
                        ...
                        "cnn": nn.Sequential(
                            ...
                            if is_leaf
                            else OrderedDict(
                                {
                                    ...
                                    "conv2d_2": factory_conv2d(1, 1, 3, pool_size=None),
                                }
                            )
                        ),
...
toskip = all - {"flower_head", dataset_name}
...
im = Image.open(gen[20])
display(im)
hm = model(im, transform=True, cnn=True).squeeze(dim=(0, 1)).cpu().detach().numpy()
sns.heatmap(hm, vmax=8, vmin=0, cmap="viridis", square=True)
