                ...
                OrderedDict(
                    {
                        ...
                        "inference": nn.Sequential(
                            OrderedDict(
                                {
                                    "lin": factory_linear(4, 1, nn.Sigmoid),
                                }
                            )
                        ),
                    }
    ...
    def setup(self, stage=None):
        ...
        if stage == "test":  # or stage is None:
            all_test = ImageFolder(
                debug(str((all_classes_dir / ".." / "val").resolve())),
                transform=self.dataset_transform,
                target_transform=lambda x: torch.tensor(
                    x == dir_index, dtype=torch.float
                ),
...
gen = pathlib.Path(
    "/mnt/milchior/serdaylv/data/imagenet/ILSVRC/Data/CLS-LOC/train/n10148035"
).iterdir()
next(gen)
next(gen)
next(gen)
