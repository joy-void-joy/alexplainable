...
def load_frozen(model, ckpt, **kwds):
    ...
    def __init__(
        ...
        def get_constants(is_leaf: bool, is_root: bool) -> "TreeCNN.Constants":
            return TreeCNN.Constants(
                ...
                crop_size=256 if is_leaf else 512,
            )
                ...
                OrderedDict(
                    {
                        "cnn": nn.Sequential(
                            OrderedDict(
                                {
                                    "conv2d_1": factory_conv2d(3, 16, 5),
                                    "conv2d_2": factory_conv2d(16, 32, 3),
                                    "conv2d_3": factory_conv2d(32, 1, 3),
                                }
                            )
                            if is_leaf
                            else OrderedDict(
                                {
                                    "conv2d_1": factory_conv2d(
                                        len(self.cnn_children), 1, 3, pool_size=None
                                    ),
                                }
                            )
                        ),
    ...
    def setup(self, stage=None):
        ...
        def balance_dataset(dataset):
            if not self.is_root:
                ...
                groups = list(groupby(dataset.samples, lambda i: i[1]))
                subset_correct = groups[dir_index][1]
                subset_incorrect = [next(i[1]) for i in groups if i[0] != dir_index]
...
all = ["disk", "petal", "flower_head", "leaf", "stem", dataset_name]
toskip = []
...
torchvision.transforms.functional.to_pil_image(a[0][0])
